"""Level 7.5.3.3: fixed-state Memory parameter-group causal intervention."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_3_3"
STATIC_PREREGISTRATION = LEVEL_DIR / "preregistration.json"
PARENT_RESULT = ROOT / "experiments/level7_5_3_2/formal/result.json"
PARENT_RESULT_SHA256 = "bc1490388243c38a011d72b7dec272345d13ae034f1847e3cd21dc778e3cb6cd"
PARENT_CLASSIFICATION = "optimizer_and_data_stream_both_causal"
EXACT_ARM = PARENT_EXACT_ARM
INTERVENTION_ARMS = [
    "freeze_l2_memory_pathway",
    "freeze_l3_memory_pathway",
    "freeze_l2_update_gate",
    "freeze_l3_update_gate",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7533000
CONFIRM_DATASET_SEED = 7533001
ARM_GROUPS = {
    EXACT_ARM: {"kind": "none", "layer": None},
    "freeze_l2_memory_pathway": {"kind": "memory_pathway", "layer": 2},
    "freeze_l3_memory_pathway": {"kind": "memory_pathway", "layer": 3},
    "freeze_l2_update_gate": {"kind": "update_gate", "layer": 2},
    "freeze_l3_update_gate": {"kind": "update_gate", "layer": 3},
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_3_3/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.3.3 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_3_3/formal":
        raise ValueError("Formal Level 7.5.3.3 output path is locked")
...
def validate_static_protocol(protocol: dict[str, Any]) -> None:
    ...
    if protocol.get("arms") != [
        {"name": name, **ARM_GROUPS[name]} for name in ALL_ARMS
    ]:
...
def selected_parameter_names(model: torch.nn.Module, arm: str) -> list[str]:
    ...
    if group["kind"] == "none":
        return []
    block = int(group["layer"]) - 1
    prefixes = [f"blocks.{block}.memory.update_gate."]
    if group["kind"] == "memory_pathway":
        prefixes = [
            f"blocks.{block}.memory.",
            f"blocks.{block}.memory_read.",
            f"blocks.{block}.memory_fusion_gate.",
        ]
...
def run_training_arm(
    ...
    if resume_path.exists() and not args.force:
        ...
        for name, parameter in model.named_parameters():
            parameter.requires_grad_(name not in names)
...
def diagnose(
    ...
    broad_l2 = effect_counts["freeze_l2_memory_pathway"]
    broad_l3 = effect_counts["freeze_l3_memory_pathway"]
    gate_l2 = effect_counts["freeze_l2_update_gate"]
    gate_l3 = effect_counts["freeze_l3_update_gate"]
    l3_gate_matches = sum(
        a["material_effect"] == b["material_effect"]
        for a, b in zip(by_arm["freeze_l3_memory_pathway"], by_arm["freeze_l3_update_gate"])
    )
    l2_gate_matches = sum(
        a["material_effect"] == b["material_effect"]
        for a, b in zip(by_arm["freeze_l2_memory_pathway"], by_arm["freeze_l2_update_gate"])
    )
    if not integrity_passed:
        classification = "formal_integrity_failed_parameter_interpretation_closed"
    elif broad_l3 >= 2 and gate_l3 >= 2 and l3_gate_matches == 4 and broad_l2 == 0:
        classification = "l3_update_gate_dominant"
    elif broad_l3 >= 2 and gate_l3 <= 1 and broad_l2 == 0:
        classification = "l3_non_gate_memory_pathway"
    elif broad_l2 >= 1 and broad_l3 <= 1 and gate_l2 >= 1 and l2_gate_matches == 4:
        classification = "l2_update_gate_dominant"
    elif broad_l2 >= 1 and broad_l3 >= 2:
        classification = "distributed_l2_l3_memory_pathway"
    elif broad_l2 or broad_l3:
        classification = "layer_specific_memory_pathway"
    else:
        classification = "no_registered_memory_group_effect"
    diagnosis = {
        "classification": classification,
        "effect_counts": effect_counts,
        "layer2_gate_matches": l2_gate_matches,
        "layer3_gate_matches": l3_gate_matches,
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    colors = {
        EXACT_ARM: "#333333",
        "freeze_l2_memory_pathway": "#d1495b",
        "freeze_l3_memory_pathway": "#0077b6",
        "freeze_l2_update_gate": "#f4a261",
        "freeze_l3_update_gate": "#2a9d8f",
    }
    labels = {
        EXACT_ARM: "exact reference",
        "freeze_l2_memory_pathway": "freeze L2 Memory",
        "freeze_l3_memory_pathway": "freeze L3 Memory",
        "freeze_l2_update_gate": "freeze L2 gate",
        "freeze_l3_update_gate": "freeze L3 gate",
    }
    ...
    figure.suptitle("Level 7.5.3.3 Memory parameter-group causal intervention", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_3_3/smoke/result.json", result)
    print("LEVEL7_5_3_3_SMOKE_PASS" if result["passed"] else "LEVEL7_5_3_3_SMOKE_FAIL")
