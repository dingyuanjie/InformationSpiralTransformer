"""Level 7.5.4: fixed-state combination-freeze Memory intervention."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_4"
...
INTERVENTION_ARMS = ["single_l2_slot_queries","single_l2_write_core","single_l2_read_fusion","single_l3_slot_queries","single_l3_write_core","single_l3_read_fusion","pair_l2_all","pair_l3_all","cross_slot_queries","cross_write_core","cross_read_fusion","triple_l2_l3_slot_queries","triple_l2_l3_write_core","triple_l2_l3_read_fusion","all_memory_groups"]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7540000
CONFIRM_DATASET_SEED = 7540001
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_4/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.4 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_4/formal":
        raise ValueError("Formal Level 7.5.4 output path is locked")
...
def selected_parameter_names(model: torch.nn.Module, arm: str) -> list[str]:
    if arm == EXACT_ARM:
        return []
    groups = []
    if arm.startswith('single_l2_'): groups=[(2,arm[len('single_l2_'):])]
    elif arm.startswith('single_l3_'): groups=[(3,arm[len('single_l3_'):])]
    elif arm == 'pair_l2_all': groups=[(2,k) for k in ('slot_queries','write_core','read_fusion')]
    elif arm == 'pair_l3_all': groups=[(3,k) for k in ('slot_queries','write_core','read_fusion')]
    elif arm.startswith('cross_'): groups=[(2,arm[6:]),(3,arm[6:])]
    elif arm.startswith('triple_l2_l3_'): groups=[(2,arm[13:]),(3,arm[13:])]
    elif arm == 'all_memory_groups': groups=[(l,k) for l in (2,3) for k in ('slot_queries','write_core','read_fusion')]
    prefixes=[]
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.4 Memory parameter-group causal intervention", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_4/smoke/result.json", result)
    print("level7_5_4_SMOKE_PASS" if result["passed"] else "level7_5_4_SMOKE_FAIL")
