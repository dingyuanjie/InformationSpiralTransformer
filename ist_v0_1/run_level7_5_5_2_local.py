"""Level 7.5.5.2: L3 slot/read-fusion interaction recovery experiment."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_5_2"
STATIC_PREREGISTRATION = LEVEL_DIR / "preregistration.json"
PARENT_RESULT = ROOT / "experiments/level7_5_5_1/formal/result.json"
PARENT_RESULT_SHA256 = "430789e4b8190da32645593ede0d54cd9a8d47f70e474bf1a6c91f5f468d02a7"
PARENT_CLASSIFICATION = "minimum_sufficient_recovery_candidates_found"
EXACT_ARM = PARENT_EXACT_ARM
INTERVENTION_ARMS = [
    "all_memory_frozen",
    "restore_l3_read_fusion", "restore_l3_slot_queries", "restore_l3_slot_read_fusion",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7552000
CONFIRM_DATASET_SEED = 7552001
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_5_2/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.5.2 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_5_2/formal":
        raise ValueError("Formal Level 7.5.5.2 output path is locked")
...
def selected_parameter_names(model: torch.nn.Module, arm: str) -> list[str]:
    ...
    restored=set()
    if arm == 'restore_l3_slot_read_fusion': restored={(3,'slot_queries'),(3,'read_fusion')}
...
def diagnose(
    ...
    for index, spec in enumerate(SOURCE_SPECS):
        slot=recovery_gains['restore_l3_slot_queries'][index]
        read=recovery_gains['restore_l3_read_fusion'][index]
        joint=recovery_gains['restore_l3_slot_read_fusion'][index]
        interaction.append({'source_id':spec['id'],'slot_gain':slot,'read_fusion_gain':read,'joint_gain':joint,'synergy_over_additive':joint-slot-read,'stabilizes_read_fusion':joint >= read})
    ...
    diagnosis = {
        ...
        "slot_read_fusion_interaction": interaction,
        "stabilization_count": sum(row['stabilizes_read_fusion'] for row in interaction),
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.5.2 L3 slot/read-fusion interaction", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_5_2/smoke/result.json", result)
    print("level7_5_5_2_SMOKE_PASS" if result["passed"] else "level7_5_5_2_SMOKE_FAIL")
