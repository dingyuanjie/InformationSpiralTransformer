"""Level 7.5.5.5: L3 slot AdamW-state causal decomposition."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_5_5"
STATIC_PREREGISTRATION = LEVEL_DIR / "preregistration.json"
PARENT_RESULT = ROOT / "experiments/level7_5_5_4/formal/result.json"
PARENT_RESULT_SHA256 = "368ca2e00de087c36ded6ddd118e91096c9dfc7ab36fad bffd38fab170fb55ac".replace(" ", "")
PARENT_CLASSIFICATION = "minimum_sufficient_recovery_candidates_found"
EXACT_ARM = PARENT_EXACT_ARM
INTERVENTION_ARMS = [
    "all_memory_frozen",
    "restore_l3_read_fusion",
    "read_slot_dose_025_keep", "read_slot_dose_025_reset_m1", "read_slot_dose_025_reset_m12",
    "read_slot_dose_100_keep", "read_slot_dose_100_reset_m1", "read_slot_dose_100_reset_m12",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7555000
CONFIRM_DATASET_SEED = 7555001
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_5_5/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.5.5 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_5_5/formal":
        raise ValueError("Formal Level 7.5.5.5 output path is locked")
...
def run_training_arm(
    ...
    for step in range(last_step + 1, args.training_steps + 1):
        if threshold is not None and step == threshold + 1:
            ...
            print(f"seed={spec['seed']} arm={arm} opened_slots_after={threshold} dose={dose:.2f} reset={reset_mode}", flush=True)
        model.train()
        probe.train()
        random_step(
            model,
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.5.5 L3 slot AdamW-state decomposition", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_5_5/smoke/result.json", result)
    print("level7_5_5_5_SMOKE_PASS" if result["passed"] else "level7_5_5_5_SMOKE_FAIL")
