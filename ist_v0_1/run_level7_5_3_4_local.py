"""Level 7.5.3.4: fixed-state Memory parameter-group causal intervention."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_3_4"
...
INTERVENTION_ARMS = [
    "freeze_l2_slot_queries",
    "freeze_l3_slot_queries",
    "freeze_l2_write_core",
    "freeze_l3_write_core",
    "freeze_l2_read_fusion",
    "freeze_l3_read_fusion",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7534000
CONFIRM_DATASET_SEED = 7534001
ARM_GROUPS = {
    EXACT_ARM: {"kind": "none", "layer": None},
    "freeze_l2_slot_queries": {"kind": "slot_queries", "layer": 2},
    "freeze_l3_slot_queries": {"kind": "slot_queries", "layer": 3},
    "freeze_l2_write_core": {"kind": "write_core", "layer": 2},
    "freeze_l3_write_core": {"kind": "write_core", "layer": 3},
    "freeze_l2_read_fusion": {"kind": "read_fusion", "layer": 2},
    "freeze_l3_read_fusion": {"kind": "read_fusion", "layer": 3},
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_3_4/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.3.4 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_3_4/formal":
        raise ValueError("Formal Level 7.5.3.4 output path is locked")
...
def selected_parameter_names(model: torch.nn.Module, arm: str) -> list[str]:
    ...
    if group["kind"] == "slot_queries":
        prefixes = [f"blocks.{block}.memory.slot_queries"]
    elif group["kind"] == "write_core":
        prefixes = [f"blocks.{block}.memory.encoder.", f"blocks.{block}.memory.memory_key.", f"blocks.{block}.memory.memory_attention."]
    elif group["kind"] == "read_fusion":
        prefixes = [f"blocks.{block}.memory_read.", f"blocks.{block}.memory_fusion_gate."]
    else:
        prefixes = []
...
def diagnose(
    ...
    elif len(set(l3_effects.values())) == 1 and any(l3_effects.values()): classification = "distributed_fine_memory"
    elif len(dominant_l3) == 1 and dominant_l3[0] == "slot_queries": classification = "l3_slot_queries_dominant"
    elif len(dominant_l3) == 1 and dominant_l3[0] == "write_core": classification = "l3_write_core_dominant"
    elif len(dominant_l3) == 1 and dominant_l3[0] == "read_fusion": classification = "l3_read_fusion_dominant"
    elif any(l2_effects.values()) and not any(l3_effects.values()): classification = "l2_fine_memory_group_effect"
    elif any(l2_effects.values()) or any(l3_effects.values()): classification = "distributed_fine_memory"
    else:
        classification = "no_registered_memory_group_effect"
    diagnosis = {
        "classification": classification,
        "effect_counts": effect_counts,
        "layer2_effect_counts": l2_effects,
        "layer3_effect_counts": l3_effects,
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.3.4 Memory parameter-group causal intervention", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_3_4/smoke/result.json", result)
    print("level7_5_3_4_SMOKE_PASS" if result["passed"] else "level7_5_3_4_SMOKE_FAIL")
