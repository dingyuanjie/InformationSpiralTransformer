"""Level 7.5.5.4: delayed L3 slot gradient-dose experiment."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_5_4"
STATIC_PREREGISTRATION = LEVEL_DIR / "preregistration.json"
PARENT_RESULT = ROOT / "experiments/level7_5_5_3/formal/result.json"
PARENT_RESULT_SHA256 = "d0f704290c5de6361f13061e8bffb41dff13b1e2bdf090f514467769e2a6cb46"
PARENT_CLASSIFICATION = "minimum_sufficient_recovery_candidates_found"
EXACT_ARM = PARENT_EXACT_ARM
INTERVENTION_ARMS = [
    "all_memory_frozen",
    "restore_l3_read_fusion",
    "read_slot_dose_010", "read_slot_dose_025", "read_slot_dose_050", "read_slot_dose_100",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7554000
CONFIRM_DATASET_SEED = 7554001
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_5_4/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.5.4 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_5_4/formal":
        raise ValueError("Formal Level 7.5.5.4 output path is locked")
...
def slot_gradient_dose(arm: str) -> float:
    return int(arm.rsplit('_', 1)[1]) / 100.0 if arm.startswith('read_slot_dose_') else 1.0
        ...
        if threshold is not None and step == threshold + 1:
            for name, parameter in model.named_parameters():
                if name in delayed_names:
                    parameter.requires_grad_(True)
                    hook_handles.append(parameter.register_hook(lambda grad, scale=dose: grad * scale))
            print(f"seed={spec['seed']} arm={arm} opened_slots_after={threshold} dose={dose:.2f}", flush=True)
        model.train()
        probe.train()
        random_step(
            model,
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.5.4 L3 slot gradient dose", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_5_4/smoke/result.json", result)
    print("level7_5_5_4_SMOKE_PASS" if result["passed"] else "level7_5_5_4_SMOKE_FAIL")
