"""Level 7.6.6.2: targeted causal rescue of low-performing 32K IST seeds."""
...
CONDITIONS = {
    "intact": (1.0, 1.0, 1.0),
    "l3_scale_0_50": (1.0, 1.0, 0.50),
    "l3_scale_0_32": (1.0, 1.0, 0.32),
    "l3_scale_0_20": (1.0, 1.0, 0.20),
    "l2_l3_scale_0_32": (1.0, 0.32, 0.32),
    "all_layers_scale_0_32": (0.32, 0.32, 0.32),
}
SAMPLES = 30
...
def configure(model, scales: tuple[float, float, float]) -> None:
    for block, scale in zip(model.blocks, scales):
        block.memory.propagation_scale = scale
...
def evaluate(model, seed: int, window_name: str, condition: str,
    ...
    configure(model, CONDITIONS[condition])
    correctness = []
    realized_ratios = []
    for sample in range(SAMPLES):
        # Same examples for every condition make all causal contrasts paired.
        example_seed = 766200000 + seed * 1000 + (0 if window_name == "near" else 500) + sample
        ...
        realized_ratios.append([float(block.memory.last_diagnostics["propagation_ratio"].float().mean().cpu())
                                for block in model.blocks])
    return {"seed": seed, "seed_group": "low" if seed in LOW_SEEDS else "high",
            "window": window_name, "range": list(WINDOWS[window_name]),
            "condition": condition, "scales": list(CONDITIONS[condition]),
            "samples": SAMPLES, "correct": sum(correctness),
            "accuracy": sum(correctness) / SAMPLES, "correctness": correctness,
            "mean_realized_propagation_ratio_by_layer": [
                sum(row[layer] for row in realized_ratios) / len(realized_ratios) for layer in range(3)]}
...
def main() -> int:
    ...
    parser.add_argument("--output", default="experiments/level7_6_6_2/formal")
    ...
    protocol = {"variant": "ist-full", "length": LENGTH, "windows": WINDOWS,
                ...
                "paired_examples": True, "primary_endpoint": "low-group accuracy change vs intact",
                "specificity_endpoint": "high-group accuracy harm vs intact"}
    if args.dry_run:
        ...
        for window_name in WINDOWS:
            for condition in CONDITIONS:
                ...
                print(f"seed={seed} group={row['seed_group']} window={window_name} "
                      f"condition={condition} accuracy={row['accuracy']:.2%}", flush=True)
        del model
    ...
    for group, seeds in (("low", LOW_SEEDS), ("high", HIGH_SEEDS)):
        for window_scope in ("near", "far", "combined"):
            ...
            for condition in CONDITIONS:
                treatment = [value for run in runs if run["seed"] in seeds and run["window"] in selected_windows
                             and run["condition"] == condition for value in run["correctness"]]
    ...
    # Correct the five intervention families separately for each group/window endpoint.
