"""Level 7.5.5.3: delayed L3 slot unfreezing interaction experiment."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_5_3"
STATIC_PREREGISTRATION = LEVEL_DIR / "preregistration.json"
PARENT_RESULT = ROOT / "experiments/level7_5_5_2/formal/result.json"
PARENT_RESULT_SHA256 = "8ef17a9401f1e240757890bec85c4d20b84040ad9ed96c67cc67cf9abc5d6507"
PARENT_CLASSIFICATION = "minimum_sufficient_recovery_candidates_found"
EXACT_ARM = PARENT_EXACT_ARM
INTERVENTION_ARMS = [
    "all_memory_frozen",
    "restore_l3_read_fusion", "restore_l3_slot_read_fusion",
    "read_then_slot_300", "read_then_slot_600",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7553000
CONFIRM_DATASET_SEED = 7553001
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_5_3/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.5.3 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_5_3/formal":
        raise ValueError("Formal Level 7.5.5.3 output path is locked")
...
def selected_parameter_names(model: torch.nn.Module, arm: str) -> list[str]:
    ...
    restored=set()
    if arm in ('restore_l3_slot_read_fusion','read_then_slot_300','read_then_slot_600'): restored={(3,'slot_queries'),(3,'read_fusion')}
...
def delayed_slot_names(model: torch.nn.Module, arm: str) -> list[str]:
    if not arm.startswith('read_then_slot_'):
        return []
...
def delayed_unfreeze_step(arm: str) -> int | None:
    return int(arm.rsplit('_', 1)[1]) if arm.startswith('read_then_slot_') else None
...
def run_training_arm(
    ...
    for step in range(last_step + 1, args.training_steps + 1):
        if threshold is not None and step == threshold + 1:
            ...
            print(f"seed={spec['seed']} arm={arm} opened_slots_after={threshold}", flush=True)
        model.train()
        probe.train()
        random_step(
            model,
...
def diagnose(
    ...
    for index, spec in enumerate(SOURCE_SPECS):
        ...
        joint=recovery_gains['restore_l3_slot_read_fusion'][index]
        delayed300=recovery_gains['read_then_slot_300'][index]
        delayed600=recovery_gains['read_then_slot_600'][index]
        interaction.append({'source_id':spec['id'],'read_fusion_gain':read,'joint_from_start_gain':joint,'delayed_300_gain':delayed300,'delayed_600_gain':delayed600,'delay_300_improves_joint':delayed300 > joint,'delay_600_improves_joint':delayed600 > joint,'delay_300_matches_read_only':delayed300 >= read,'delay_600_matches_read_only':delayed600 >= read})
    ...
    diagnosis = {
        ...
        "delayed_slot_interaction": interaction,
        "delay_300_improvement_count": sum(row['delay_300_improves_joint'] for row in interaction),
        "delay_600_improvement_count": sum(row['delay_600_improves_joint'] for row in interaction),
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.5.3 delayed L3 slot unfreezing", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_5_3/smoke/result.json", result)
    print("level7_5_5_3_SMOKE_PASS" if result["passed"] else "level7_5_5_3_SMOKE_FAIL")
