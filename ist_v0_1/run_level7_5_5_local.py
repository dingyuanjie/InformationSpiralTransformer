"""Level 7.5.5: minimum-sufficient Memory recovery intervention."""
...
LEVEL_DIR = ROOT / "experiments" / "level7_5_5"
STATIC_PREREGISTRATION = LEVEL_DIR / "preregistration.json"
PARENT_RESULT = ROOT / "experiments/level7_5_4/formal/result.json"
PARENT_RESULT_SHA256 = "633b4e8de21f9c5fb80da3eaa0c553d77800bb733dbbdafa9fadcc408e2343c2"
PARENT_CLASSIFICATION = "distributed_fine_memory"
EXACT_ARM = PARENT_EXACT_ARM
INTERVENTION_ARMS = [
    "all_memory_frozen",
    "restore_l2_slot_queries", "restore_l2_write_core", "restore_l2_read_fusion",
    "restore_l3_slot_queries", "restore_l3_write_core", "restore_l3_read_fusion",
    "restore_all_l2", "restore_all_l3",
    "restore_cross_slot_queries", "restore_cross_write_core", "restore_cross_read_fusion",
]
ALL_ARMS = [EXACT_ARM, *INTERVENTION_ARMS]
SCREEN_DATASET_SEED = 7550000
CONFIRM_DATASET_SEED = 7550001
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="experiments/level7_5_5/formal")
...
def formal_protocol_check(args: argparse.Namespace) -> None:
    ...
    if mismatches:
        raise ValueError(f"Formal Level 7.5.5 protocol is locked: {mismatches}")
    if args.output != "experiments/level7_5_5/formal":
        raise ValueError("Formal Level 7.5.5 output path is locked")
...
def selected_parameter_names(model: torch.nn.Module, arm: str) -> list[str]:
    ...
    restored=set()
    if arm.startswith('restore_l2_'): restored={(2,arm[len('restore_l2_'):])}
...
def plot_parameter_effects(outcomes: list[dict[str, Any]], path: Path) -> None:
    ...
    figure.suptitle("Level 7.5.5 Memory parameter-group causal intervention", y=0.98)
...
def run_smoke(args: argparse.Namespace) -> int:
    ...
    atomic_save(ROOT / "experiments/level7_5_5/smoke/result.json", result)
    print("level7_5_5_SMOKE_PASS" if result["passed"] else "level7_5_5_SMOKE_FAIL")
