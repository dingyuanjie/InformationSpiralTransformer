...
def matched_width(target):
    ...
    for width in range(64,129,8):
        count=params(StandardTransformer(19,width,3,8,2048,0.0,"rope"))
        candidates.append((abs(count-target),width,count))
    return min(candidates)[1:]

def build(name,matched):
    if name=="transformer-64": return StandardTransformer(19,64,3,8,2048,0.0,"rope")
    if name=="transformer-matched": return StandardTransformer(19,matched,3,8,2048,0.0,"rope")
    return InformationSpiralTransformer(19,64,3,2048,"rope",True)
...
def run(name,seed,args,device,dtype,matched,root):
    ...
    for index,(length,needle_range,steps) in enumerate(STAGES[start_stage:],start_stage+1):
        for step in range(1,steps+1):
            ...
            with torch.autocast(device_type=device.type,dtype=dtype,enabled=device.type=="cuda"):
                ...
                loss=q+0.5*local+(0.1*model.memory_diversity_loss() if name=="ist-c" else 0)
    ...
    long_tests=[evaluate(model,n,n-3,args.long_eval_batches,args.long_eval_batch_size,device,dtype) for n in (512,1024,2048)]
    ...
    result={"variant":name,"seed":seed,"parameters":params(model),"accuracy_auc":auc,"seconds":time.perf_counter()-started,
...
def main():
    p=argparse.ArgumentParser(description="Level 5A.2 fixed-step parameter-matched comparison")
    p.add_argument("--variants",nargs="+",default=["transformer-64","transformer-matched","ist-c"],choices=["transformer-64","transformer-matched","ist-c"])
    ...
    p.add_argument("--output",default="experiments/level5a2/formal"); p.add_argument("--force",action="store_true"); args=p.parse_args()
    if not torch.cuda.is_available(): raise RuntimeError("CUDA GPU required")
    device=torch.device("cuda"); dtype=torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    target=params(InformationSpiralTransformer(19,64,3,2048,"rope",True)); matched,matched_params=matched_width(target)
    ...
    for name in args.variants:
        ...
        summary.append({"variant":name,"parameters":selected[0]["parameters"],"mean_auc":statistics.mean(r["accuracy_auc"] for r in selected),
          ...
          "mean_long_accuracy":{str(n):statistics.mean(r["long_tests"][index]["query_accuracy"] for r in selected)
                                for index,n in enumerate((512,1024,2048))}})
